/*
 * ============================================================
 *  Arduino Nano Music Player - Touch Sensor Edition
 *
 *  Hardware:
 *    DFPlayer Mini  → SoftwareSerial RX=10, TX=11
 *    SH1106 1.3" OLED I2C → A4 (SDA), A5 (SCL)
 *    TTP223 Touch Sensors (active HIGH)
 *      Play/Pause → D2 | Next → D3
 *    Buttons (active LOW, INPUT_PULLUP)
 *      Prev → D4 | Shuffle → D5 | Repeat → D6
 *
...
// ── Touch sensor debounce (active HIGH, edge detection) ──────
const uint8_t TOUCH_PINS[]      = {TOUCH_PLAY_PAUSE, TOUCH_NEXT};
bool          lastTouch[2]      = {LOW, LOW};
uint32_t      lastTouchTime[2]  = {0, 0};

bool touchPressed(uint8_t i) {
  bool reading = digitalRead(TOUCH_PINS[i]);
  uint32_t now = millis();
  if (reading == HIGH && lastTouch[i] == LOW && (now - lastTouchTime[i]) > 60) {
    lastTouch[i]     = HIGH;
    lastTouchTime[i] = now;
    return true;
  }
  if (reading == LOW) lastTouch[i] = LOW;
  return false;
}

// ── Push button debounce (active LOW, INPUT_PULLUP) ──────────
const uint8_t BTN_PINS[]      = {BTN_PREV, BTN_SHUFFLE, BTN_REPEAT};
bool          lastBtn[3]      = {HIGH, HIGH, HIGH};
uint32_t      lastBtnTime[3]  = {0, 0, 0};

bool buttonPressed(uint8_t i) {
  bool reading = digitalRead(BTN_PINS[i]);
  if (reading != lastBtn[i]) {
    lastBtnTime[i] = millis();
    lastBtn[i]     = reading;
  }
  if ((millis() - lastBtnTime[i]) > 60 && reading == LOW) {
...
void showChecklist(bool dfOK, bool sdOK) {
  // Check if any push button is responding
  bool btnsOK = false;
  for (uint8_t i = 0; i < 3; i++) {
    if (digitalRead(BTN_PINS[i]) == HIGH) { btnsOK = true; break; }
  }
...
// ── Setup ─────────────────────────────────────────────────────
void setup() {
  // Touch sensors as INPUT (active HIGH, no pull-up needed)
  pinMode(TOUCH_PLAY_PAUSE, INPUT);
  pinMode(TOUCH_NEXT,       INPUT);

  // Push buttons as INPUT_PULLUP (active LOW)
...
// ── Loop ──────────────────────────────────────────────────────
uint32_t lastDraw = 0;
const uint32_t DRAW_INTERVAL = 200;
...
void loop() {
  ...
  // ── Touch: Next (D3) ────────────────────────────────────
  if (touchPressed(1) && dfOK) {
    uint8_t next = shuffleOn ? randomTrack()
                             : (currentTrack < totalTracks ? currentTrack + 1 : 1);
    playTrack(next);
  }

  // ── Button: Prev (D4) ───────────────────────────────────
  if (buttonPressed(0) && dfOK) {
    uint8_t prev = shuffleOn ? randomTrack()
                             : (currentTrack > 1 ? currentTrack - 1 : totalTracks);
    playTrack(prev);
  }

  // ── Button: Shuffle (D5) ────────────────────────────────
  if (buttonPressed(1)) shuffleOn = !shuffleOn;
